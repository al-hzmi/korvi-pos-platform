...
- `a7553e4d31fae66a087e241382bca60420f3afb7`: governed restaurant waste/spoilage authority VERIFIED on isolated branch `product/post-v1-restaurant-waste`. CI `35734109879` is green end-to-end (193 test files / 2,359 tests passed; 26 files / 381 tests skipped). PostgreSQL proof `35734109870` is green under restricted runtime/migrator separation: 38 live files discovered / 34 runtime-live, `restaurant-waste-live.test.ts` 5/5 passed, migration rehearsals + rollback fault harness passed, and final PostgreSQL-backed verify passed (194 files / 2,360 tests; 25 files / 380 skipped). Waste is a causal document, never an adjustment shortcut: server-authorized waste/spoilage reasons consume tracked stock through the shared inventory/cost ledger, preserve known/unknown cost provenance, refuse physical over-consumption regardless of ordinary sale oversell policy, enforce FORCE-RLS/composite tenant FKs, and roll document/movement/cost/audit/idempotency back atomically on failure.
- `d70c04cddc1e37ca6e19cd1326baae37f5b56d53`: cashier split/mixed-tender workflow VERIFIED on isolated branch `product/post-v1-restaurant-split-payment`. Standard CI `35754600795` is green through dependency pins, audit, formatting, lint, invariants, Prisma, build, typecheck and tests (194 test files / 2,370 tests passed; 26 files / 381 live-only tests skipped). The POS now composes cash plus bounded electronic tenders in integer minor units, captures only payment scheme + external approval reference, persists the exact tender intent through offline queue/replay, freezes payment state while outcome is ambiguous, and still delegates all pricing, settlement, change and fiscal authority to the existing server/domain checkout path. This closes the cashier workflow gap only; restaurant bill/order split and merge authority remains open.

Immediate next action:

**Restaurant bill/order split + merge authority → course sequencing / safe post-fire delta semantics**, continuing from the verified KDS + recipe production + waste + split-tender foundations without weakening transaction, financial, inventory or fiscal boundaries.
