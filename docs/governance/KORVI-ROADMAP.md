...
- **M5 — VERIFIED** on `product/post-v1-migration-engine@0fad77ff837fe4677c67bd74cab50be384bda068`: Opening Inventory is end-to-end for tenant-scoped `branchCode + sku + openingQuantity` business-key input with CSV/XLSX inspection/mapping/preview, dry run, controlled commit, row-level errors, audit/idempotency/provenance, Arabic merchant UI/template/error export and restricted-runtime isolation proof. Commit re-resolves branch/SKU authority server-side, requires `settings.manage + inventory.adjust`, refuses client UUID/cost authority, rechecks pristine stock at commit and writes one causal `migration-opening-stock` inventory movement rather than fabricating balances or purchases. Opening quantity enters costing as explicit UNKNOWN provenance/value unless separate authoritative cost semantics are defined. Full CI `35720975124` is green with 201 test files / 2,374 tests passed and 30 files / 395 live-only tests skipped; PostgreSQL/RLS proof `35720975187` is green with 6 files / 26 tests under the restricted non-superuser, non-BYPASSRLS runtime.
- **M6 — VERIFIED** on `product/post-v1-migration-engine-m6@f8b1c0bd253b7668b3da370c307cfa5dcf269857`: the first explicit conflict/update strategy is Customer `update-existing-by-phone`, and it is deliberately opt-in while the default remains `reject`. The strategy requires a mapped phone business key, never accepts `customerId` or guessed name/email/VAT matching, resolves only inside the authenticated tenant, plans same-tenant phone matches during dry run, and re-resolves again under tenant lock at commit. Commit reuses Korvi's authoritative customer update writer, changes only fields actually mapped from the source, preserves unmapped existing fields, records created vs updated outcomes separately, and retains idempotent retry semantics; legacy reject jobs keep their previous request-hash behavior. Product and other migration domains remain on their previously verified conflict semantics rather than gaining speculative update authority. Full CI `35724243049` is green with 201 test files / 2,375 tests passed and 30 files / 397 live-only tests skipped; restricted-runtime PostgreSQL/RLS proof `35724243105` is green with 6 files / 28 tests passed.
- **M7 — DEFERRED:** system-specific adapters only from real customer demand; no speculative adapter is required for the verified migration baseline.

**CUSTOMER MIGRATION READINESS — VERIFIED.** The baseline P0 onboarding gate is now satisfied for products, categories/product mapping, customers, suppliers and opening inventory from supported CSV/XLSX flows with mapping, validation, preview, dry run, controlled commit, error reporting, audit/provenance, tenant isolation and idempotency. M6/M7 are enhancements and do not regress this verified baseline.
