...
- **M6 — ADOPTED:** richer explicit conflict/update strategies only where Korvi already has a deterministic tenant-scoped business key and authoritative update semantics; no client-controlled internal IDs and no guessed matching.
- **M7 — DEFERRED:** system-specific adapters only from real customer demand.
