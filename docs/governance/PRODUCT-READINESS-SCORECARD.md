...
| # | Gate | State | Evidence |
|---|---|---|---|
| 46 | Exact-head CI enforces dependency pins, audit, format, lint, invariants, build, typecheck and tests | CLOSED | Gate 44 implementation CI `34664871538` succeeded on `f501be79...`; earlier cashier/Phase-2 release-track CI evidence remains retained |
...
Verified Product M1 merchant-workflow follow-on:

- branch `product/post-v1-migration-engine`;
- SHA `e1a407565642ea5fcfda6820f475c0c833a6cbea`;
- CI `35612574884` green through dependency pins, audit, formatting, lint, invariants, Prisma, build, typecheck and tests;
...
M6 Customer conflict/update strategy is now **VERIFIED** on `product/post-v1-migration-engine-m6@f8b1c0bd253b7668b3da370c307cfa5dcf269857`. Full CI `35724243049` is green with 201 test files / 2,375 tests passed and 30 files / 397 live-only tests skipped. Restricted-runtime PostgreSQL/RLS proof `35724243105` is green with 6 files / 28 tests passed. The only M6 update strategy currently claimed is explicit Customer `update-existing-by-phone`: default reject remains unchanged; phone must be mapped; no internal customer id or guessed matching is accepted; dry run resolves same-tenant phone matches; commit re-resolves under tenant lock and uses the authoritative customer writer; only mapped fields are changed and unmapped existing fields are preserved. Other migration domains retain their previously verified conflict semantics.

**Customer Migration Readiness remains VERIFIED for the adopted baseline.** Products, categories/product mapping, customers, suppliers and opening inventory retain supported CSV/XLSX mapping, validation, preview, dry run, controlled commit, row-level errors, audit/provenance, tenant isolation and idempotency. M6 strengthens deterministic conflict handling without changing the baseline gate or the canonical 88/100 release score.

Current migration next action: **M7 DEFERRED — system-specific adapters only from real customer demand; no speculative adapter work is active.**
