...
| Migration Engine / customer data onboarding | B0/B1 | C0/C1 | ADOPTED P0; M0 VERIFIED; Product M1 VERIFIED on `2442de56` including merchant workflow and PostgreSQL RLS proof. **M2 VERIFIED** on `product/post-v1-migration-engine@9ce8afc54c4336111f4489a4fb03b651bc9fe6c3`, CI `35631102915` (195 test files / 2,335 tests passed; 27 files / 382 live-only tests skipped) plus restricted-runtime PostgreSQL isolation proof `35630229494`: category CSV/XLSX vertical slice, Arabic merchant UI/template/formula-safe error export, and server-only tenant-scoped `categoryNameAr → categoryId` resolution re-applied at dry-run and commit. Files/browser never gain category UUID authority; missing/inactive categories fail row-level; no auto-create or cross-tenant leakage. **M3 Customer Migration = VERIFIED** on `fffd2e7d870826428064a85d4e52973aa86ea465`, PR CI `35672537533` (197 test files / 2,349 tests passed; 28 files / 387 live-only tests skipped) plus restricted-runtime PostgreSQL proof `35672535019` (4 files / 18 tests passed). Customer CSV/XLSX migration now covers the actual supported fields only (`nameAr`, `nameEn`, `phone`, `email`, `vatNumber`) with deterministic mapping/validation, preview, dry run, authoritative controlled commit, idempotency/audit/provenance, Arabic UI/template/error export and cross-tenant/race isolation proof. **M4 Supplier Migration = IN PROGRESS**. CUSTOMER MIGRATION READINESS remains IN PROGRESS until suppliers + opening inventory are proven end-to-end. | Migration/Onboarding | Data Integrity, Security, Tenant Isolation, Idempotency, Audit, Reconciliation |
