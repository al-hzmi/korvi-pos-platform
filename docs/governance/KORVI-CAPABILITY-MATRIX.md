...
| Migration Engine / customer data onboarding | B0/B1 | C0/C1 | ADOPTED P0; M0 VERIFIED. Product M1 backend/API slice VERIFIED on `product/post-v1-migration-engine@b07c050b`: bounded CSV/XLSX inspection, deterministic mapping/preview data, tenant-scoped reviewed jobs, dry-run catalogue conflict detection, reject-only policy, controlled create-only commit through existing product authority, audit/idempotency/provenance, original spreadsheet row tracking and row-level results. M1 remains IN PROGRESS pending merchant mapping/preview/error UI and downloadable error export. CUSTOMER MIGRATION READINESS is not GREEN until products + customers + suppliers + opening inventory are proven end-to-end. | Migration/Onboarding | Data Integrity, Security, Tenant Isolation, Idempotency, Audit, Reconciliation |
