...
| Migration Engine / customer data onboarding | B0/B1 | C0/C1 | ADOPTED P0; M0 VERIFIED; Product M1 VERIFIED on `2442de56` including merchant workflow and PostgreSQL RLS proof. Category M2 backend/API is VERIFIED on `product/post-v1-migration-engine@c1a97498`, CI `35623377323` (194 test files / 2,327 tests passed): deterministic CSV/XLSX mapping/preview, row validation, tenant-scoped jobs, dry run, create-only category commit through existing authority, audit/idempotency/provenance. M2 remains IN PROGRESS pending product→category mapping, category merchant UI/template/error export and PostgreSQL tenant-isolation proof. CUSTOMER MIGRATION READINESS remains IN PROGRESS until products + customers + suppliers + opening inventory are all proven end-to-end. | Migration/Onboarding | Data Integrity, Security, Tenant Isolation, Idempotency, Audit, Reconciliation |
