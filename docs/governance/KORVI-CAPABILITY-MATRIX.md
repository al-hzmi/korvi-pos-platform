...
| Kitchen routing/KDS | B1 | C1 | KDS CORE IMPLEMENTED / VERIFIED on `product/post-v1-restaurant-foundation@11238975`: branch-scoped Preparation Stations, deterministic product→station routing, idempotent order fire, queued→preparing→ready→served lifecycle with timing/revision/audit, dedicated KDS UI, explicit POS «إرسال للمطبخ», retry-safe firing and server-side post-fire lock preventing unsafe line edits/cancellation. Preparation payloads remain structurally non-fiscal. Advanced course sequencing and safe delta/re-fire semantics remain open. | Restaurant/KDS | Operational, Offline |
