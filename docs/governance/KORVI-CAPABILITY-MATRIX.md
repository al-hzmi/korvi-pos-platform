...
| Migration Engine / customer data onboarding | B0/B1 | C0/C1 | ADOPTED P0; M0 VERIFIED. Product M1 backend/API slice VERIFIED on `b07c050b`, and merchant mapping/preview/dry-run/commit/error-export workflow VERIFIED on `product/post-v1-migration-engine@e1a40756` with CI `35612574884` (191 test files / 2,315 tests passed). CSV/XLSX source inspection, deterministic mapping, preview, reviewed jobs, dry run, reject-only conflict policy, create-only controlled commit, audit/idempotency/provenance, physical row tracking and downloadable CSV problem export are implemented. Product M1 is VERIFIED on `product/post-v1-migration-engine@2442de56`: the merchant workflow remains green and PostgreSQL isolation proof `35615195979` proves forced RLS, foreign-job/row invisibility, non-disclosure of Tenant B conflicts, cross-tenant mutation refusal and tenant-consistent FK protection under the restricted runtime role. CUSTOMER MIGRATION READINESS is still IN PROGRESS until products + customers + suppliers + opening inventory are proven end-to-end. | Migration/Onboarding | Data Integrity, Security, Tenant Isolation, Idempotency, Audit, Reconciliation |
