...
--
-- The queue freezes the exact signed XML/hash/UUID before the first outbound
-- byte. A network ambiguity is durable and cannot be silently retransmitted.
...
CREATE TABLE "zatca_invoice_submissions" (
  ...
  CONSTRAINT "zatca_invoice_submissions_secret_handle_check"
    CHECK (
      char_length("secretProvider") BETWEEN 1 AND 500 AND
      char_length("secretId") BETWEEN 1 AND 500
    ),
    ...
    CHECK (
      (
        ...
        (
          ...
          ("mode" = 'clearance' AND "authorityStatus" = 'CLEARED' AND "clearedInvoiceXml" IS NOT NULL)
        )
      ) OR (
        "state" = 'rejected' AND "attemptCount" = 1 AND
        "requestStartedAt" IS NOT NULL AND "resolvedAt" IS NOT NULL AND
        "httpStatus" IS NOT NULL AND "authorityStatus" IS NULL AND
        "rejectionCode" IS NOT NULL AND char_length("rejectionCode") BETWEEN 1 AND 500 AND
...
CREATE OR REPLACE FUNCTION guard_zatca_invoice_submission()
RETURNS trigger
LANGUAGE plpgsql
AS $$
DECLARE
...
BEFORE INSERT OR UPDATE ON "zatca_invoice_submissions"
