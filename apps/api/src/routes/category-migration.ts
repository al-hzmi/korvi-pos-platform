import { z } from 'zod';
import { MAX_CATEGORY_IMPORT_BYTES } from '../migration/category-import-service.js';
import type {
  CategoryMigrationFailureReason,
...
const TARGET_FIELD = z.enum(['nameAr', 'nameEn', 'sortOrder']);
const DELIMITER = z.enum([',', ';', '\t']);
const SOURCE = z
  .object({
    csvText: z.string().min(1).max(MAX_CATEGORY_IMPORT_BYTES),
...
const STATUS: Readonly<Record<CategoryMigrationFailureReason, number>> = {
  'file-too-large': 413,
  'invalid-csv': 422,
  'invalid-xlsx': 422,
  'empty-file': 422,
...
export function registerCategoryMigrationRoutes(
  app: FastifyInstance,
  options: { readonly service: MerchantCategoryMigrationService; readonly guards: Guards },
): void {
