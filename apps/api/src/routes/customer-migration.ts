import { z } from 'zod';
import { MAX_CUSTOMER_IMPORT_BYTES } from '../migration/customer-import-service.js';
import type {
  MerchantCustomerMigrationService,
  CustomerMigrationFailureReason,
...
const STATUS: Readonly<Record<CustomerMigrationFailureReason, number>> = {
  'file-too-large': 413,
  'invalid-csv': 422,
  'invalid-xlsx': 422,
  'empty-file': 422,
...
export function registerCustomerMigrationRoutes(
  app: FastifyInstance,
  options: { readonly service: MerchantCustomerMigrationService; readonly guards: Guards },
): void {
  ...
  app.get(
    '/v1/admin/migrations/customers/jobs/:jobId',
    { preHandler: manage },
    async (request, reply) => {
      const principal = principalOf(request);
      if (principal === undefined) return reply.code(401).send({ error: 'unauthenticated' });
  ...
  app.get(
    '/v1/admin/migrations/customers/jobs/:jobId/rows',
    { preHandler: manage },
    async (request, reply) => {
      const principal = principalOf(request);
      if (principal === undefined) return reply.code(401).send({ error: 'unauthenticated' });
  ...
  app.post(
    '/v1/admin/migrations/customers/jobs/:jobId/dry-run',
    { preHandler: manage },
    async (request, reply) => {
      const principal = principalOf(request);
      if (principal === undefined) return reply.code(401).send({ error: 'unauthenticated' });
  ...
  app.post(
    '/v1/admin/migrations/customers/jobs/:jobId/commit',
    {
      preHandler: [
        guards.requireSession,
        guards.requirePermission('settings.manage'),
        guards.requirePermission('customer.write'),
      ],
    },
    async (request, reply) => {
      const principal = principalOf(request);
