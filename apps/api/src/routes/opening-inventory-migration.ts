...
const TARGET_FIELD = z.enum(['branchCode', 'sku', 'openingQuantity']);
const DELIMITER = z.enum([',', ';', '\t']);
const SOURCE = z
  .object({
    csvText: z.string().min(1).max(MAX_OPENING_INVENTORY_IMPORT_BYTES),
...
const STATUS: Readonly<Record<OpeningInventoryMigrationFailureReason, number>> = {
  'file-too-large': 413,
  'invalid-csv': 422,
  'invalid-xlsx': 422,
  'empty-file': 422,
...
export function registerOpeningInventoryMigrationRoutes(
  app: FastifyInstance,
  options: { readonly service: MerchantOpeningInventoryMigrationService; readonly guards: Guards },
): void {
  ...
  app.get(
    '/v1/admin/migrations/opening-inventory/jobs/:jobId',
    { preHandler: manage },
    async (request, reply) => {
      const principal = principalOf(request);
      if (principal === undefined) return reply.code(401).send({ error: 'unauthenticated' });
  ...
  app.get(
    '/v1/admin/migrations/opening-inventory/jobs/:jobId/rows',
    { preHandler: manage },
    async (request, reply) => {
      const principal = principalOf(request);
      if (principal === undefined) return reply.code(401).send({ error: 'unauthenticated' });
  ...
  app.post(
    '/v1/admin/migrations/opening-inventory/jobs/:jobId/dry-run',
    { preHandler: manage },
    async (request, reply) => {
      const principal = principalOf(request);
      if (principal === undefined) return reply.code(401).send({ error: 'unauthenticated' });
  ...
  app.post(
    '/v1/admin/migrations/opening-inventory/jobs/:jobId/commit',
    {
      preHandler: [
