...
describe.skipIf(url === '')('Gate 13 device enrollment authority, live', () => {
  ...
  async function evidenceCounts(
    tenantId: string,
  ): Promise<{ enrollments: number; audits: number }> {
  ...
  it('proves enrollment, isolation, idempotency, public-key-only persistence and revocation', async () => {
    ...
    const fingerprintA = createHash('sha256')
      .update(`fingerprint-a:${installationId}`)
      .digest('hex');
    ...
    const crossTenantRead = await withTenant(
      prisma,
      tenantB.id,
      async (tx) =>
    ...
    expect(
      persisted.columns.some(({ columnName }) => /private.*key|key.*private/i.test(columnName)),
    ).toBe(false);
