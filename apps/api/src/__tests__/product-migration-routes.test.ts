...
import type { ProductImportSummary } from '@korvi/database';
...
function service(): MerchantProductMigrationService {
  return {
...
describe('product migration HTTP authority', () => {
  it('requires settings.manage before inspection and never calls the service on refusal', async () => {
    ...
    const response = await server.inject({
      method: 'POST',
      url: '/v1/admin/migrations/products/inspect-csv',
  ...
  it('keeps dry-run separate from commit and commit requires product.write too', async () => {
    ...
    const dry = await manageOnly.inject({
      method: 'POST',
      url: `/v1/admin/migrations/products/jobs/${JOB}/dry-run`,
      headers: { cookie: COOKIE, origin: ORIGIN },
      payload: {},
    });
    const deniedCommit = await manageOnly.inject({
      method: 'POST',
      url: `/v1/admin/migrations/products/jobs/${JOB}/commit`,
      headers: { cookie: COOKIE, origin: ORIGIN },
