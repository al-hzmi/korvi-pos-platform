...
export const MAX_SUPPLIER_IMPORT_DATA_ROWS = 50_000;
export const MAX_SUPPLIER_IMPORT_PREVIEW_ROWS = 20;

export type SupplierCsvDelimiter = ',' | ';' | '\t';

export type SupplierMigrationFailureReason =
  SupplierImportRefusal | 'file-too-large' | 'invalid-csv' | 'invalid-xlsx' | 'empty-file';

export type SupplierMigrationResult<T> =
  | { readonly outcome: 'success'; readonly value: T }
  | { readonly outcome: 'failure'; readonly reason: SupplierMigrationFailureReason };

export interface CsvSupplierSourceInput {
  readonly csvText: string;
  readonly fileName: string | null;
  readonly sourceSystem: string | null;
  readonly delimiter: SupplierCsvDelimiter;
}

export interface CreateCsvSupplierImportRequest extends CsvSupplierSourceInput {
  readonly operationId: string;
  readonly mapping: readonly SupplierColumnMapping[];
}

export interface XlsxSupplierSourceInput {
  readonly xlsxBase64: string;
  readonly fileName: string | null;
  readonly sourceSystem: string | null;
}

export interface CreateXlsxSupplierImportRequest extends XlsxSupplierSourceInput {
  readonly operationId: string;
  readonly mapping: readonly SupplierColumnMapping[];
}

export interface SupplierSourceInspection {
  readonly sourceSha256: string;
  readonly fileBytes: number;
  readonly totalRows: number;
  readonly header: readonly SupplierMappingSuggestion[];
  readonly mappingIssues: ReturnType<typeof validateSupplierMappings>;
...
export interface MerchantSupplierMigrationService {
  inspectCsv(
    principal: AuthenticatedPrincipal,
    input: CsvSupplierSourceInput,
  ): Promise<SupplierMigrationResult<SupplierSourceInspection>>;
  createCsvJob(
    principal: AuthenticatedPrincipal,
    request: CreateCsvSupplierImportRequest,
  ): Promise<SupplierMigrationResult<SupplierImportSummary>>;
  inspectXlsx(
    principal: AuthenticatedPrincipal,
    input: XlsxSupplierSourceInput,
  ): Promise<SupplierMigrationResult<SupplierSourceInspection>>;
  createXlsxJob(
    principal: AuthenticatedPrincipal,
    request: CreateXlsxSupplierImportRequest,
  ): Promise<SupplierMigrationResult<SupplierImportSummary>>;
  readJob(
    principal: AuthenticatedPrincipal,
    jobId: string,
  ): Promise<SupplierMigrationResult<SupplierImportSummary | null>>;
  rows(
    principal: AuthenticatedPrincipal,
    jobId: string,
    options: {
  ...
  ): Promise<SupplierMigrationResult<SupplierImportRowPage | null>>;
  dryRun(
    principal: AuthenticatedPrincipal,
    jobId: string,
  ): Promise<SupplierMigrationResult<SupplierImportSummary>>;
...
function parseXlsxSource(input: XlsxSupplierSourceInput): SupplierMigrationResult<{
  ...
  if (decoded.length > MAX_SUPPLIER_IMPORT_BYTES) {
    return { outcome: 'failure', reason: 'file-too-large' };
  }
  try {
...
async function databaseAttempt<T>(work: () => Promise<T>): Promise<SupplierMigrationResult<T>> {
  try {
    return { outcome: 'success', value: await work() };
  } catch (error) {
    if (error instanceof SupplierImportRefusedError) {
...
export function createMerchantSupplierMigrationService(
  prisma: PrismaClient,
  ...
  return {
    async inspectCsv(principal, input) {
      ...
      const header = suggestSupplierMappings(sheet.rows[0]!);
      ...
      return {
        ...
        value: {
          ...
          header,
          mappingIssues: validateSupplierMappings(sheet.rows[0]!, suggestedMapping),
          previewRows: sheet.rows
            .slice(1, MAX_SUPPLIER_IMPORT_PREVIEW_ROWS + 1)
            .map((cells, index) => ({
    ...
    async readJob(principal, jobId) {
      requirePrincipalPermission(principal, 'settings.manage');
      return databaseAttempt(() => readSupplierImportJob(prisma, scopeOf(principal), jobId));
    },

    async rows(principal, jobId, options) {
      requirePrincipalPermission(principal, 'settings.manage');
      return databaseAttempt(() =>
        readSupplierImportRows(prisma, scopeOf(principal), jobId, options),
      );
    },
    ...
    async dryRun(principal, jobId) {
      requirePrincipalPermission(principal, 'settings.manage');
      return databaseAttempt(() =>
        dryRunSupplierImport(prisma, scopeOf(principal), { userId: principal.userId }, jobId),
      );
    },
