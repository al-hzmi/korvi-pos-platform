...
export type CustomerMigrationResult<T> =
  | { readonly outcome: 'success'; readonly value: T }
  | { readonly outcome: 'failure'; readonly reason: CustomerMigrationFailureReason };

export interface CsvCustomerSourceInput {
  readonly csvText: string;
  readonly fileName: string | null;
  readonly sourceSystem: string | null;
  readonly delimiter: CustomerCsvDelimiter;
}
...
export interface CustomerSourceInspection {
  ...
  readonly header: readonly CustomerMappingSuggestion[];
  readonly mappingIssues: ReturnType<typeof validateCustomerMappings>;
...
export interface MerchantCustomerMigrationService {
  inspectCsv(
    principal: AuthenticatedPrincipal,
    input: CsvCustomerSourceInput,
  ): Promise<CustomerMigrationResult<CustomerSourceInspection>>;
  createCsvJob(
    principal: AuthenticatedPrincipal,
    request: CreateCsvCustomerImportRequest,
  ): Promise<CustomerMigrationResult<CustomerImportSummary>>;
  inspectXlsx(
    principal: AuthenticatedPrincipal,
    input: XlsxCustomerSourceInput,
  ): Promise<CustomerMigrationResult<CustomerSourceInspection>>;
  createXlsxJob(
    principal: AuthenticatedPrincipal,
    request: CreateXlsxCustomerImportRequest,
  ): Promise<CustomerMigrationResult<CustomerImportSummary>>;
  readJob(
    principal: AuthenticatedPrincipal,
    jobId: string,
  ): Promise<CustomerMigrationResult<CustomerImportSummary | null>>;
  rows(
    principal: AuthenticatedPrincipal,
    jobId: string,
    options: {
  ...
  ): Promise<CustomerMigrationResult<CustomerImportRowPage | null>>;
  dryRun(
    principal: AuthenticatedPrincipal,
    jobId: string,
  ): Promise<CustomerMigrationResult<CustomerImportSummary>>;
...
function parseSource(input: CsvCustomerSourceInput): CustomerMigrationResult<{
...
function parseXlsxSource(input: XlsxCustomerSourceInput): CustomerMigrationResult<{
...
async function databaseAttempt<T>(work: () => Promise<T>): Promise<CustomerMigrationResult<T>> {
  try {
    return { outcome: 'success', value: await work() };
  } catch (error) {
    if (error instanceof CustomerImportRefusedError) {
...
export function createMerchantCustomerMigrationService(
  prisma: PrismaClient,
  ...
  return {
    async inspectCsv(principal, input) {
      ...
      const header = suggestCustomerMappings(sheet.rows[0]!);
    ...
    async readJob(principal, jobId) {
      requirePrincipalPermission(principal, 'settings.manage');
      return databaseAttempt(() => readCustomerImportJob(prisma, scopeOf(principal), jobId));
    },

    async rows(principal, jobId, options) {
      requirePrincipalPermission(principal, 'settings.manage');
      return databaseAttempt(() =>
        readCustomerImportRows(prisma, scopeOf(principal), jobId, options),
      );
    },
    ...
    async dryRun(principal, jobId) {
      requirePrincipalPermission(principal, 'settings.manage');
      return databaseAttempt(() =>
        dryRunCustomerImport(prisma, scopeOf(principal), { userId: principal.userId }, jobId),
      );
    },
