...
import {
  ...
  SUPPLIER_MIGRATION_FIELD_LABELS,
...
const ALL_FIELDS = Object.keys(SUPPLIER_MIGRATION_FIELD_LABELS) as SupplierMigrationTargetField[];

type PreparedSource =
  | { readonly format: 'csv'; readonly value: CsvSupplierMigrationSource }
  | { readonly format: 'xlsx'; readonly value: XlsxSupplierMigrationSource };
...
function rowTone(classification: SupplierMigrationRowResult['classification']): string {
...
export function SupplierMigrationPanel({
  api,
  canCommit,
  disabled = false,
  onCommandLockChange,
  ...
  const [inspection, setInspection] = useState<SupplierMigrationInspection | null>(null);
  const [mapping, setMapping] = useState<readonly SupplierMigrationMapping[]>([]);
  const [job, setJob] = useState<SupplierMigrationSummary | null>(null);
  const [problemRows, setProblemRows] = useState<readonly SupplierMigrationRowResult[]>([]);
  ...
  const mappingProblems = useMemo(() => supplierMigrationMappingProblems(mapping), [mapping]);
  ...
  const inspect = async (): Promise<void> => {
    ...
    try {
      ...
      let result: SupplierMigrationInspection;
      if (format === 'csv') {
        const value: CsvSupplierMigrationSource = {
        ...
        result = await api.inspectSupplierMigrationCsv(value);
      } else {
        const value: XlsxSupplierMigrationSource = {
          xlsxBase64: arrayBufferToBase64(await file.arrayBuffer()),
          fileName: file.name,
          sourceSystem: sourceSystem.trim() === '' ? null : sourceSystem.trim(),
        };
        nextPrepared = { format, value };
        result = await api.inspectSupplierMigrationXlsx(value);
      }
  ...
  async function loadProblems(jobId: string): Promise<void> {
    setBusy('problems');
    try {
      const page = await api.supplierMigrationRows(jobId, {
        limit: 200,
        problemsOnly: true,
      });
    ...
    try {
      const result =
        prepared.format === 'csv'
          ...
          : await api.createSupplierMigrationXlsxJob({
              ...prepared.value,
              operationId,
              mapping,
            });
      setJob(result);
      setPending(null);
      setNotice('أُنشئت مراجعة قابلة للتتبع. لم يُكتب أي مورد بعد.');
      onCommandLockChange(false);
    } catch (error) {
  ...
  const executeCommit = async (operationId: string): Promise<void> => {
    ...
    onCommandLockChange(true);
    try {
      const result = await api.commitSupplierMigration(job.id, operationId);
      setJob(result);
      setPending(null);
  ...
  const exportProblems = async (): Promise<void> => {
    ...
    setFailure(null);
    try {
      const rows: SupplierMigrationRowResult[] = [];
      let after: number | null = null;
      for (let pageNumber = 0; pageNumber < 200; pageNumber += 1) {
        const page = await api.supplierMigrationRows(job.id, {
          limit: 500,
      ...
      downloadText('korvi-supplier-import-errors.csv', supplierMigrationProblemsCsv(rows));
    ...
    <div className="flex flex-col gap-4">
      <CardSurface className="p-5">
        <div className="flex flex-wrap items-start justify-between gap-3">
          <div>
            <h2 className="text-base font-semibold text-foreground">استيراد الموردين</h2>
          ...
          <Button
            ...
            onClick={() =>
              downloadText('korvi-suppliers-template.csv', SUPPLIER_MIGRATION_TEMPLATE_CSV)
            }
          >
            قالب الموردين CSV
          </Button>
                    ...
                    <tr key={column.sourceColumn} className="border-b border-border">
                      ...
                      <td className="px-3 py-3">
                        <select
                          ...
                          disabled={commandLocked}
                          onChange={(event) => {
                            const value = event.target.value as SupplierMigrationTargetField | '';
                            setMapping((current) =>
                              current.map((item) =>
                        ...
                        >
                          ...
                          {ALL_FIELDS.map((field) => (
                            <option key={field} value={field}>
                              {SUPPLIER_MIGRATION_FIELD_LABELS[field]}
                            </option>
                          ))}
                        </select>
                      </td>
      ...
      {job === null ? null : (
        <CardSurface className="p-5">
          <div className="flex flex-wrap items-start justify-between gap-3">
            <div>
              <h3 className="font-semibold text-foreground">نتيجة مهمة ترحيل الموردين</h3>
