...
import {
  ...
  PRODUCT_MIGRATION_FIELD_LABELS,
...
const ALL_FIELDS = Object.keys(PRODUCT_MIGRATION_FIELD_LABELS) as ProductMigrationTargetField[];

type PreparedSource =
  | { readonly format: 'csv'; readonly value: CsvProductMigrationSource }
  | { readonly format: 'xlsx'; readonly value: XlsxProductMigrationSource };
...
function rowTone(classification: ProductMigrationRowResult['classification']): string {
...
export function MigrationPanel({
  ...
  const [inspection, setInspection] = useState<ProductMigrationInspection | null>(null);
  const [mapping, setMapping] = useState<readonly ProductMigrationMapping[]>([]);
  const [job, setJob] = useState<ProductMigrationSummary | null>(null);
  const [problemRows, setProblemRows] = useState<readonly ProductMigrationRowResult[]>([]);
  ...
  const inspect = async (): Promise<void> => {
    ...
    try {
      ...
      let result: ProductMigrationInspection;
      if (format === 'csv') {
        const value: CsvProductMigrationSource = {
        ...
        result = await api.inspectProductMigrationCsv(value);
      } else {
        const value: XlsxProductMigrationSource = {
          xlsxBase64: arrayBufferToBase64(await file.arrayBuffer()),
          fileName: file.name,
          sourceSystem: sourceSystem.trim() === '' ? null : sourceSystem.trim(),
        };
        nextPrepared = { format, value };
        result = await api.inspectProductMigrationXlsx(value);
      }
  ...
  async function loadProblems(jobId: string): Promise<void> {
    setBusy('problems');
    try {
      const page = await api.productMigrationRows(jobId, {
        limit: 200,
        problemsOnly: true,
      });
  ...
  const executeCreate = async (operationId: string): Promise<void> => {
    ...
    try {
      const result =
        prepared.format === 'csv'
          ? await api.createProductMigrationCsvJob({
              ...prepared.value,
              operationId,
              mapping,
            })
          : await api.createProductMigrationXlsxJob({
              ...prepared.value,
              operationId,
              mapping,
            });
      setJob(result);
      setPending(null);
      setNotice('أُنشئت مراجعة قابلة للتتبع. لم تُكتب أي أصناف بعد.');
      onCommandLockChange(false);
    } catch (error) {
  ...
  const executeDryRun = async (): Promise<void> => {
    ...
    onCommandLockChange(true);
    try {
      const result = await api.dryRunProductMigration(job.id);
      setJob(result);
      setPending(null);
      setNotice('اكتمل الفحص التجريبي باستخدام منطق التعارض الحقيقي دون إنشاء أصناف.');
      onCommandLockChange(false);
  ...
  const executeCommit = async (operationId: string): Promise<void> => {
    ...
    onCommandLockChange(true);
    try {
      const result = await api.commitProductMigration(job.id, operationId);
      setJob(result);
      setPending(null);
  ...
  const exportProblems = async (): Promise<void> => {
    ...
    setFailure(null);
    try {
      const rows: ProductMigrationRowResult[] = [];
      let after: number | null = null;
      for (let pageNumber = 0; pageNumber < 200; pageNumber += 1) {
        const page = await api.productMigrationRows(job.id, {
          limit: 500,
      ...
      downloadText('korvi-product-import-errors.csv', productMigrationProblemsCsv(rows));
    ...
    <div className="flex flex-col gap-4">
      <CardSurface className="p-5">
        <div className="flex flex-wrap items-start justify-between gap-3">
          <div>
            <h2 className="text-base font-semibold text-foreground">استيراد المنتجات</h2>
          ...
          <Button
            ...
            onClick={() =>
              downloadText('korvi-products-template.csv', PRODUCT_MIGRATION_TEMPLATE_CSV)
            }
          >
            قالب المنتجات CSV
          </Button>
                    ...
                    <tr key={column.sourceColumn} className="border-b border-border">
                      ...
                      <td className="px-3 py-3">
                        <select
                          ...
                          disabled={commandLocked}
                          onChange={(event) => {
                            const value = event.target.value as ProductMigrationTargetField | '';
                            setMapping((current) =>
                              current.map((item) =>
                        ...
                        >
                          ...
                          {ALL_FIELDS.map((field) => (
                            <option key={field} value={field}>
                              {PRODUCT_MIGRATION_FIELD_LABELS[field]}
                            </option>
                          ))}
                        </select>
                      </td>
      ...
      {job === null ? null : (
        <CardSurface className="p-5">
          <div className="flex flex-wrap items-start justify-between gap-3">
            <div>
              <h3 className="font-semibold text-foreground">نتيجة مهمة الترحيل</h3>
            ...
            {job.status === 'dry-run' ? (
              canCommit ? (
                ...
                <StatusNote tone="warning">
                  الاعتماد يحتاج صلاحية كتابة المنتجات، بينما الفحص والمراجعة متاحان لك.
                </StatusNote>
              )
            ) : null}
            <Button
