...
import {
  ...
  CUSTOMER_MIGRATION_FIELD_LABELS,
...
const ALL_FIELDS = Object.keys(CUSTOMER_MIGRATION_FIELD_LABELS) as CustomerMigrationTargetField[];

type PreparedSource =
  | { readonly format: 'csv'; readonly value: CsvCustomerMigrationSource }
  | { readonly format: 'xlsx'; readonly value: XlsxCustomerMigrationSource };
...
function rowTone(classification: CustomerMigrationRowResult['classification']): string {
...
export function CustomerMigrationPanel({
  api,
  canCommit,
  disabled = false,
  onCommandLockChange,
  ...
  const [job, setJob] = useState<CustomerMigrationSummary | null>(null);
  const [problemRows, setProblemRows] = useState<readonly CustomerMigrationRowResult[]>([]);
  ...
  const mappingProblems = useMemo(() => customerMigrationMappingProblems(mapping), [mapping]);
  ...
  const inspect = async (): Promise<void> => {
    ...
    try {
      ...
      let result: CustomerMigrationInspection;
      if (format === 'csv') {
        const value: CsvCustomerMigrationSource = {
        ...
        result = await api.inspectCustomerMigrationCsv(value);
      } else {
        const value: XlsxCustomerMigrationSource = {
          xlsxBase64: arrayBufferToBase64(await file.arrayBuffer()),
          fileName: file.name,
          sourceSystem: sourceSystem.trim() === '' ? null : sourceSystem.trim(),
        };
        nextPrepared = { format, value };
        result = await api.inspectCustomerMigrationXlsx(value);
      }
  ...
  async function loadProblems(jobId: string): Promise<void> {
    setBusy('problems');
    try {
      const page = await api.customerMigrationRows(jobId, {
        limit: 200,
        problemsOnly: true,
      });
  ...
  const executeCreate = async (operationId: string): Promise<void> => {
    ...
    try {
      ...
      setJob(result);
      setPending(null);
      setNotice('أُنشئت مراجعة قابلة للتتبع. لم يُكتب أي عميل بعد.');
      onCommandLockChange(false);
    } catch (error) {
  ...
  const executeCommit = async (operationId: string): Promise<void> => {
    ...
    onCommandLockChange(true);
    try {
      const result = await api.commitCustomerMigration(job.id, operationId);
      setJob(result);
      setPending(null);
  ...
  const exportProblems = async (): Promise<void> => {
    ...
    setFailure(null);
    try {
      const rows: CustomerMigrationRowResult[] = [];
      let after: number | null = null;
      for (let pageNumber = 0; pageNumber < 200; pageNumber += 1) {
        const page = await api.customerMigrationRows(job.id, {
          limit: 500,
      ...
      downloadText('korvi-customer-import-errors.csv', customerMigrationProblemsCsv(rows));
    ...
    <div className="flex flex-col gap-4">
      <CardSurface className="p-5">
        <div className="flex flex-wrap items-start justify-between gap-3">
          <div>
            <h2 className="text-base font-semibold text-foreground">استيراد العملاء</h2>
          ...
          <Button
            ...
            onClick={() =>
              downloadText('korvi-customers-template.csv', CUSTOMER_MIGRATION_TEMPLATE_CSV)
            }
          >
            قالب العملاء CSV
          </Button>
                    ...
                    <tr key={column.sourceColumn} className="border-b border-border">
                      ...
                      <td className="px-3 py-3">
                        <select
                          ...
                          disabled={commandLocked}
                          onChange={(event) => {
                            const value = event.target.value as CustomerMigrationTargetField | '';
                            setMapping((current) =>
                              current.map((item) =>
                        ...
                        >
                          ...
                          {ALL_FIELDS.map((field) => (
                            <option key={field} value={field}>
                              {CUSTOMER_MIGRATION_FIELD_LABELS[field]}
                            </option>
                          ))}
                        </select>
                      </td>
            ...
            {job.status === 'dry-run' ? (
              canCommit ? (
                ...
                <StatusNote tone="warning">
                  الاعتماد يحتاج صلاحية كتابة العملاء، بينما الفحص والمراجعة متاحان لك.
                </StatusNote>
              )
            ) : null}
            <Button
